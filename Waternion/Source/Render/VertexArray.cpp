...
namespace Waternion {
    ...
    VertexArray::VertexArray(const Vertex* vertices, int32_t numOfVertices, const int32_t* indices, int32_t numOfIndices) {
        // Gen VAO
        glGenVertexArrays(1, &mArrayBufferID);
        glBindVertexArray(mArrayBufferID);
        // Gen VBO
        ...
        glEnableVertexAttribArray(0);
        glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, TexCoords)); // Texture coordinates
        glEnableVertexAttribArray(1);
        glBindVertexArray(0);
    }
